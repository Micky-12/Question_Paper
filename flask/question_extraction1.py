    ...
    with open(csv_file, 'a', newline='', encoding='utf-8') as file:
        ...
        for question in questions:
            ...
            if (q2[0].isdigit()) and question[q1]>0:
                q_text = list(question.keys())[0]
                q_marks = question[q_text]
                count = exist_question(csv_file, question)
                if count > 1:
                    # If the question already exists, increment its count attribute
                    with open(csv_file, 'r', newline='', encoding='utf-8') as read_file:
                        reader = csv.reader(read_file)
                        rows = list(reader)
                    ...
                    with open(csv_file, 'w', newline='', encoding='utf-8') as write_file:
                        ...
                        #writer.writerow(['Question', 'CourseCode', 'CourseName', 'Marks', 'Count'])

                        for row in rows:
                            if row[0] == q_text:
                                row[4] = int(row[4]) + 1  # Increment the count attribute
                            writer.writerow(row)
                else:
                    # If the question does not exist, add it to the CSV with count attribute 1
                    writer.writerow([q_text, code, course, q_marks, 1])
